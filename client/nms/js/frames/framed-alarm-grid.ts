...
import CyAlarmGrid = require("../ui/grid/cy-alarm-grid");
...
$(document).ready(() => {
    ...
    $.i18n.init(option, () => {
        ...
        CyTemplates.loadTemplate("frames/framed-alarm-grid.html", function () {
            ...
            var g = new CyAlarmGrid(options);

            g.eventBus = CyIFrameMessenger.I;

            g.element.appendTo($('body'));

            g.toolbar = new CyGridComboToolbar('alarm-grid',['filter', 'copy-to-clipboard', 'acknowledge', 'viewer']);
